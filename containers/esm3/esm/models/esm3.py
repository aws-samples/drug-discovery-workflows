...
class ESM3(nn.Module, ESM3InferenceClientV1):
    """
    ...
    # The following methods are for the ESM3InferenceClientV1 interface
    ...
    def iterative_sampling(
        ...
        structure_tokens = input_tokens.structure
        coordinates = input_tokens.coordinates

        if coordinates is not None:
            if config.track == "structure":
                # TODO: Handle partial structure / active site conditioning. Maybe outside generate?
                raise ValueError(
                    "Cannot sample structure tokens when structure coordinates are provided."
                )

        output_tokens = iterative_sampling_tokens(
            self, input_tokens, config, self.tokenizers
        )

        # Add back structure tokens before decoding
        # TODO: Remove this once we can handle partial structure
        if coordinates is not None:
            if output_tokens.structure is None:
                output_tokens.structure = structure_tokens
    ...
    def encode(
        ...
        if input.coordinates is not None:
            coordinates, confidence, structure_tokens = encoding.tokenize_structure(
                input.coordinates,
        ...
        if input.function_annotations is not None:
            if input.sequence is None:
                reference_sequence = encoding.get_default_sequence(sequence_length)
            else:
        ...
        if override_structure_tokens_with_coords and coordinates is not None:
            structure_tokens = None
    ...
    def decode(
        ...
        if input.sequence is not None:
            sequence = decoding.decode_sequence(
                input.sequence, self.tokenizers.sequence
            )

        if input.structure is not None:
            if input.coordinates is not None and override_structure_tokens_with_coords:
                coordinates = input.coordinates[1:-1, ...]
                confidence = (
                    input.confidence[1:1] if input.confidence is not None else None
                )
            else:
                [coordinates, confidence] = decoding.decode_structure(
                    structure_tokens=input.structure,
                    structure_decoder=self.get_structure_token_decoder(),
                    structure_tokenizer=self.tokenizers.structure,
                    sequence=sequence,
                )
        ...
        if input.secondary_structure is not None:
            secondary_structure = decoding.decode_secondary_structure(
                input.secondary_structure, self.tokenizers.secondary_structure
            )
        if input.sasa is not None:
            # TODO: Return midpoint float value for SASA instead of string with the range
            sasa = decoding.decode_sasa(input.sasa, self.tokenizers.sasa)
        if input.function is not None:
            function_track_annotations = decoding.decode_function_annotations(
                input.function,
        ...
        if input.residue_annotations is not None:
            residue_annotations = decoding.decode_residue_annotations(
                input.residue_annotations, self.tokenizers.residue_annotations
            )
        ...
        with torch.no_grad() if self.eval else contextlib.nullcontext():
            if compile:
                ...
                output = self.compile(
                    ...
                    average_plddt=(
                        torch.mean(input.confidence)
                        if input.confidence is not None
                        else None
                    ),
                    per_res_plddt=input.confidence,
            ...
            return ForwardOutput(
                ...
                embeddings=output.embeddings if return_embeddings else None,
            )
    ...
    def forward_and_sample(
        ...
        if protein_tensor.confidence is not None:
            confidence = protein_tensor.confidence
            if len(confidence.size()) == 1:
                confidence = confidence.unsqueeze(0)
            protein_tensor.confidence = confidence.to(device)
        ...
        for track in [
            ...
            if getattr(protein_tensor, track) is None:
                setattr(
                    ...
                    track,
                    (
                        self.get_default_tokens_for_track(
                            track,
                            sequence_length - 2,  # Tokenizers handle BOS and EOS tokens
                        )
                        .unsqueeze(0)
                        .to(device)
                    ),
        ...
        for track in ["sequence", "structure", "secondary_structure", "sasa"]:
            ...
            if config is None:
                track_sampling_metadata_dir[track] = None
                continue
        ...
        # Sample function separately
        config = getattr(sampling_config, "function")
        if config is None:
            track_sampling_metadata_dir["function"] = None
        else:
        ...
        # Sample residue annotations separately
        ...
        return ForwardAndSampleOutput(**forward_and_sample_output_dir)

    def get_default_tokens_for_track(
        self, track: str, sequence_length: int
    ) -> torch.Tensor:
        if track == "sequence":
            return encoding.get_default_sequence_tokens(
                sequence_length, self.tokenizers.sequence
            )
        if track == "structure":
            return encoding.get_default_structure_tokens(
                sequence_length, self.tokenizers.structure
            )
        if track == "secondary_structure":
            return encoding.get_default_secondary_structure_tokens(
                sequence_length, self.tokenizers.secondary_structure
            )
        if track == "sasa":
            return encoding.get_default_sasa_tokens(
                sequence_length, self.tokenizers.sasa
            )
        if track == "function":
            return encoding.get_default_function_tokens(
                sequence_length, self.tokenizers.function
            )
        if track == "residue_annotations":
            return encoding.get_default_residue_annotation_tokens(
                sequence_length, self.tokenizers.residue_annotations
            )
        raise ValueError(f"Unknown track: {track}")

    # TODO: Add unbatching + sampling + unbatching utils
    def _sample_track(
        self,
        logits: torch.Tensor,
        tokens: torch.Tensor,
    ...
    #     torch.jit.save(traced_encoder, "encoder.pt")
    #     print("Loading encoder")
    #     traced_encoder = torch.jit.load("encoder.pt")
    ...
    #     print("Tracing transformer")

    #     traced_transformer = torch_neuronx.trace(
    #         self.transformer,
    #         (x, sequence_id, affine, affine_mask, chain_id),
    #     )
    #     print("Saving transformer")
    #     torch.jit.save(traced_transformer, "transformer.pt")
    #     print("Loading transformer")
    #     traced_transformer = torch.jit.load("transformer.pt")
    #     x, embedding = traced_transformer(
    #         x, sequence_id, affine.tensor, affine_mask, chain_id
    #     )

    #     # x, embedding = self.transformer(x, sequence_id, affine, affine_mask, chain_id)

    #     output = self.output_heads(x, embedding)
    #     # traced_decoder = torch_neuronx.trace(self.output_heads, (x, embedding))
    #     # print("Saving decoder")
    #     # torch.jit.save(traced_decoder, "decoder.pt")
    #     # print("Loading decoder")
    #     # traced_decoder = torch.jit.load("decoder.pt")
    #     # output = traced_decoder(x, embedding)

    #     print(f"sequence_logits: {output.sequence_logits.shape}")
    #     print(f"structure_logits: {output.structure_logits.shape}")
    #     print(f"secondary_structure_logits: {output.secondary_structure_logits.shape}")
    #     print(f"sasa_logits: {output.sasa_logits.shape}")
    #     print(f"function_logits: {output.function_logits.shape}")
    #     print(f"residue_logits: {output.residue_logits.shape}")
