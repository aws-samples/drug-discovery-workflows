...
class LSHTokenized:
    def __init__(
        ...
        dim: int,
        num_tables: int = 1,
        filepath: CloudPath | PathLike | None = None,
        ...
        if filepath is not None:
            filepath_maybecloud = AnyPath(filepath)
            if not filepath_maybecloud.exists():  # type: ignore
                raise FileNotFoundError(filepath_maybecloud)
            table_hyperplanes = np.load(filepath_maybecloud)  # type: ignore
