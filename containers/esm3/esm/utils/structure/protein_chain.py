...
class ProteinChain:
    ...
    def align(
        ...
        """
        aligner = Aligner(self, target, only_use_backbone)
    ...
    def rmsd(
        ...
        if isinstance(other, bs.AtomArray):
            raise ValueError(
                "Support for bs.AtomArray removed, use "
                "ProteinChain.from_atomarry for ProteinChain."
            )

        aligner = Aligner(self, other, only_compute_backbone_rmsd)
        avg_rmsd = aligner.rmsd

        if not also_check_reflection:
            return avg_rmsd

        aligner = Aligner(self, other, only_compute_backbone_rmsd, True)
    ...
    def lddt_ca(
        ...
        **kwargs,
        ...
        Arguments:
            other (ProteinChain): The other protein chain to compare to.
        ...
        """
        # TODO(scandido): In theory it would be nice to use an implementation of LDDT
        # that handles numpy arrays natively instead of requiring conversation to
        # torch tensors.
        lddt = compute_lddt_ca(
            torch.tensor(self.atom37_positions).unsqueeze(0),
            torch.tensor(other.atom37_positions).unsqueeze(0),
            torch.tensor(self.atom37_mask).unsqueeze(0),
            **kwargs,
        )
