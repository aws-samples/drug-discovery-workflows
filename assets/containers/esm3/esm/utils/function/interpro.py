...
class InterPro:
    ...
    def __init__(
        self,
        entries_path: str = C.INTERPRO_ENTRY,
        hierarchy_path: str = C.INTERPRO_HIERARCHY,
        interpro2go_path: str | None = C.INTERPRO2GO,
    ):
        """Constructs interface to query InterPro entries."""
        self.entries_path = entries_path
        self.hierarchy_graph_path = hierarchy_path
        self.interpro2go_path = interpro2go_path
...
# A shared global instantiation with default paths.
interpro = InterPro()
...
def parse_interpro_features(
    ...
    for interpro_accession, start_idx, end_idx in zip(
        ...
        # NOTE(rverkuil): Skip unintegrated Interpro labels, for now.
        if interpro_accession == "-":
            continue

        # TODO(rverkuil): Check that this passes after fixing the version mismatch.
        # entry_type = InterProEntryType[
        #     self.entry_types.loc[interpro_accession, "ENTRY_TYPE"].upper()  # pyright: ignore
        # ]
