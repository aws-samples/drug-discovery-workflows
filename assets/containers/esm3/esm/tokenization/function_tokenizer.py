...
from esm.utils.function import interpro, lsh
...
class InterProQuantizedTokenizer(EsmTokenizerBase):
    ...
    def __init__(
        self,
        lsh_path: str = C.LSH_TABLE_PATHS["8bit"],
        depth: int = 8,
        pq_vocab_size: int = 256,
        interpro_entry_csv_path: str = C.INTERPRO_ENTRY,
        tfidf_model_path: str = C.TFIDF_MODEL,
    ):
        """Constructs InterPro annotation tokenizer.
        Args:
            interpro_combination_path: path to CSV containing columns "combo", "cluster" indicating
                how to map interpro combinations to clusters / token ids.
            depth: number of tokens that are produced for the combination of InterPro
                domain annotations at each position.
            pq_vocab_size: number of values that each token token at each position may
                asusme.
            tfidf_model_path: Path to pickled TFIDFVectorizer model which vectorizes
                function text descriptions.
        """
        self.depth = depth
        self.pq_vocab_size = pq_vocab_size

        self.interpro_entry_csv_path = interpro_entry_csv_path

        nbits = self.pq_vocab_size.bit_length() - 1
        self._lsh = lsh.LSHTokenized(nbits, C.TFIDF_VECTOR_SIZE, self.depth, lsh_path)

        with Path(tfidf_model_path).open("rb") as f:
            self._tfidf_model = pickle.load(f)
            self._tfidf_model.analyzer = _tfidf_analyzer
    ...
    def vocab(self) -> list[str]:
        ...
        lsh_tokens = [f"<lsh:{i}>" for i in range(self.pq_vocab_size)]
    ...
    def tokenize(
        ...
        if p_keyword_dropout > 0:
            keyword_mask = (
                np.random.random(len(self._tfidf_model.vocabulary_)) < p_keyword_dropout
            )
        else:
            keyword_mask = None
    ...
    def _function_text_hash(
        ...
        keywords = []
        for label in labels:
            if label in self._tfidf_model.vocabulary_:
                keywords.append(label)
            else:
                raise ValueError(f"Unsupported: {label}")

        vec: sp.csr_matrix = self._tfidf_model.transform([keywords])
...
def _tfidf_analyzer(texts: list[str]) -> list[str]:
    ...
    terms = []
    for text in texts:
        terms.extend(_terms_from_text(text))
    return terms
