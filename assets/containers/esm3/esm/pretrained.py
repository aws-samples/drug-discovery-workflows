...
def ESM3_1_4B(device: torch.device | str = "cpu"):
    model = (
        ESM3(
            d_model=1536,
            n_heads=24,
    ...
    state_dict = torch.load(
        "esm/data/weights/esm3_1_4B_open_2403.pth",
        map_location=device,
    )
...
def ESM3_structure_encoder_v0(device: torch.device | str = "cpu"):
    ...
    state_dict = torch.load(
        "esm/data/weights/esm3_structure_encoder_v0.pth",
        map_location=device,
    )
    model.load_state_dict(state_dict)
    return model
...
def ESM3_structure_decoder_v0(device: torch.device | str = "cpu"):
    ...
    state_dict = torch.load(
        "esm/data/weights/esm3_structure_decoder_v0.pth",
        map_location=device,
    )
    model.load_state_dict(state_dict)
    return model
...
def ESM3_function_decoder_v0(device: torch.device | str = "cpu"):
    ...
    state_dict = torch.load(
        "esm/data/weights/esm3_function_decoder_v0.pth",
        map_location=device,
    )
    model.load_state_dict(state_dict)
    return model
