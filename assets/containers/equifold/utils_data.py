...
template_coords = os.path.join(os.path.dirname(__file__), template_coords)
if os.path.exists(template_coords):
    cg_X0 = torch.from_numpy(
        np.load(template_coords, allow_pickle=True)["x"].astype(np.float32)
    )
else:
    cg_X0 = None
...
def subtract_centroid_and_mask(cg_X, cg_T, cg_R, cg_mask, cg_atom_mask):
    ...
    masked = cg_mask == 0.0
    cg_T[masked] = 0.0  # so it's not flung off somewhere
...
def x_to_pdb(x, resnum, resname, atoms, b_factors=None) -> str:
    ...
    if b_factors is None:
        b_factors = [0] * len(x)
    for pos, resnum_, resname_, atom_name, b_factor in zip(
        x, resnum, resname, atoms, b_factors
    ):
        record_type = "ATOM"
    ...
    # )
    chain_termination_line = f"{chain_end:<6}"
...
def pdb_feats_to_data(pdb_feats, use_kabsch, real_pdb=False, dst_idx_offset=0):
    ...
    if cg_mask.sum() < 30:
        raise ValueError("Sum of CG masks is less than 30")
        ...
        if i < len(sequence) - 1:
            ...
            dst_bonds.append(
                (
                    ...
                    np.asarray(
                        [0.05, c_n_stddev * tol_factor]
                    ),  # first element is handpicked
                    np.asarray([1.0, 1.0]),
                )
            )
